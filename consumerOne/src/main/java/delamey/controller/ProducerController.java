...
public class ProducerController {
    ...
    public void sendMsg() {
        ...
        for(int i=0;i<100;i++){
            String out="发送消息"+i+number;
//            String json = JSON.toJSONString(out);
            Message msg = new Message("user-topic","white",out.getBytes());
            try {
                SendResult result = defaultProducer.send(msg);
                System.out.println("消息id:"+result.getMsgId()+":"+","+"发送状态:"+result.getSendStatus()+number);
            } catch (Exception e) {
                e.printStackTrace();
            }
